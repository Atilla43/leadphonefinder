...
const questions = [
  {
    ...
    a: "Signal Grid использует продвинутые механизмы защиты: round-robin ротация аккаунтов, лимит 30 сообщений в день на аккаунт, человекоподобные задержки между сообщениями, рассылка только в рабочие часы (10:00–17:00 МСК). Всё это минимизирует риск блокировки.",
  },
  {
    q: "Какие источники данных для поиска контактов?",
    a: "Автоматический скраппинг из Яндекс.Карт и 2ГИС через Playwright. Дополнительно — обогащение через DaData (ИНН, директор, юридическая форма). Вы также можете загрузить собственную базу в формате Excel/CSV.",
  },
  {
    q: "Насколько «умный» AI-бот?",
    a: "Бот работает на GPT-4o и GPT-4o-mini через OpenRouter. Он ведёт естественные диалоги, обрабатывает возражения, распознаёт голосовые сообщения (Whisper), извлекает контакты рефералов и адаптирует стиль общения под каждого собеседника.",
  },
  {
